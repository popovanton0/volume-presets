...
class ConfigDataSource {
    ...
    private var _volumeControlName: String?
        get() = configNode.get("volumeControlName", null).ifEmpty { null }
        set(value) = configNode.put("volumeControlName", value ?: "")
    ...
    val volumeControlName = MutableStateFlow<String?>(_volumeControlName)
    val volumePresets = MutableStateFlow<List<UInt>>(_volumePresets)

    init {
        with(GlobalScope) {
            launch { lang.collect { _lang = it } }
            launch { volumeControlName.collect { _volumeControlName = it } }
